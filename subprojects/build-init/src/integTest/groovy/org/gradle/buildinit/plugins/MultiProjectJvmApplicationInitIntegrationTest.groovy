...
class MultiProjectJvmApplicationInitIntegrationTest extends AbstractIntegrationSpec {
    ...
    def "creates multi-project application sample for #jvmLanguage with #scriptDsl build scripts"() {
        ...
        targetDir.file("buildSrc").assertHasDescendants(
            buildFile,
            "src/main/${dsl.id}/some.thing.${dsl.fileNameFor("java-common-conventions")}",
            "src/main/${dsl.id}/some.thing.${dsl.fileNameFor("java-application-conventions")}",
            "src/main/${dsl.id}/some.thing.${dsl.fileNameFor("java-library-conventions")}",
        )
